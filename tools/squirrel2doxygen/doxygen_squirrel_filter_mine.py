...
# -*- coding: utf-8 -*-	
#
# This is a modified original script by Jacob Boerema to suit IdleTTD needs.
#
...
class SquirrelFilter:
	...
	re_function_with_params = re.compile("\s*(function)\s+([a-zA-Z_]+[a-zA-Z_0-9]*)\s+\(([^)]+)\)");
	...
	def filter_part(self, part):
		...
		# re_function_with_params_2 = re.compile("\s*(?:static\s)?(?:function|constructor)\s*([a-zA-Z_]+[a-zA-Z_0-9]*)\s*\(([^)]+)\)");
		# re_function_with_params_2 = re.compile("\s*function\s+[a-zA-Z_]+[a-zA-Z_0-9]*\s*\(([^,)]+,?\s?)+?\)");
		temp = re_function_with_params_2.search(output);	
		
		# if self.current_class == "Logger":
			# self.debugforce(output + "\n---\n\n");
		...
		if temp:
				...
				for mo in re.finditer(r"([^,]+)", last):
					...
					if re.match(r'^companyID', newVal):
						# newVal = "GSCompany::CompanyID " + newVal;
						newVal = "int " + newVal;
					elif re.match(r'^storyPageID', newVal):
						# newVal = "GSStoryPage::StoryPageID " + newVal;
						newVal = "int " + newVal;
					elif re.match(r'^element', newVal):
						newVal = "GSText " + newVal;
					elif re.match(r'^pageDate', newVal):
						...
						newVal = "std::array<SummaryVehicleStats> " + newVal;
					elif re.match(r'^vehicleType', newVal):
						newVal = "int " + newVal;
						# newVal = "GSVehicle::VehicleType " + newVal;
		...
		if re.search(r'config\.nut$', self.filename):
			...
			temp = re_configvar_struct.search(output);
			if temp:

				# self.debugforce("\n\n" + output + "\n");
				output = " GSConfig " + temp.group(2);
				# output = output[:temp.start()] + " struct " + output[temp.start():];
				# output = output[:temp.start()] + " struct " + temp.group(1) + " { \n";
		
		# if re.search(r'types\.nut$', self.filename):
		# 	re_config_struct = re.compile("^(StructGSConfig)\s*=\s*");
		# 	temp = re_config_struct.search(output);
		# 	if temp:
		# 		output = output[:temp.start()] + " struct " + temp.group(1) + " { \n";
			
		# 	re_saved_struct = re.compile("^(StructNamespace)\s*=\s*");
		# 	temp = re_saved_struct.search(output);
		# 	if temp:
		# 		output = output[:temp.start()] + " struct " + temp.group(1) + " { \n";
			
			
		# 	re_saved_struct = re.compile("^(StructScriptSavedData)\s*=\s*");
		# 	temp = re_saved_struct.search(output);
		# 	if temp:
		# 		output = output[:temp.start()] + " struct " + temp.group(1) + " { \n";
		...
		# 	temp = re.search(r'\s*(WarnAfterCloseAttempts|MinSleepTime|ShortestSleepTime|ShortSleepTime|MidSleepTime|PlayerCompanyID|IdleStoryPageID|LastActiveTime|LastYearBalance|SecondToLastYearBalance|SavedScriptVersion)\s*=\s*\d+,', output);
		# 	if temp:
		# 		output = output[:temp.start(1)] + " int " + temp.group(1) + ";\n";

		# 	temp = re.search(r'\s*(ShowStatsAfterReport)\s*=\s*.*,', output);
		# 	if temp:
		# 		output = output[:temp.start(1)] + " bool " + temp.group(1) + ";\n";



		# 	re_struct = re.compile("^(StructSummaryVehicleStats|StructVehicleTypeStatsItem)\s*=\s*");
		# 	temp = re_struct.search(output);
		# 	if temp:
		# 		output = output[:temp.start()] + " struct " + temp.group(1) + " { \n";

		# 	temp = re.search(r'\s*(count|balance|idleBalance|type)\s*=\s*(0|-1),', output);
		# 	if temp:
		# 		output = output[:temp.start(1)] + " int " + temp.group(1) + ";\n";
